...
class encoderRNN(nn.Module):
    ...
    def forward(self,input):
        ...
        features = outputs.view(-1,2*self.hid_dim)
        ...
        return outputs, features, hidden
...
class decoderRNN(nn.Module):
    def __init__(self,output_size,emb_size,enc_hid_dim,dec_hid_dim,dropout,attention):
        ...
        self.out2 = nn.Linear(dec_hid_dim*3,output_size)
        init_linear_wt(self.out2)

    def forward(self, input, hidden, encoder_outputs, encoder_features, enc_padding_mask, c_t_1):
        #embedded = self.dropout(self.embedding(input))
        embedded = self.embedding(input)
